...
/* =========================================================
   ZOHO BIGIN — SAME-ORIGIN FORM (via GetQuoteForm component)
   ---------------------------------------------------------
   The old approach embedded Zoho's hosted form URL
   (us.bigin.online/.../forms/2151-get-quote) directly in a
   cross-origin iframe. That works, but being cross-origin
   means we can't measure/resize it and we can only prefill
   fields through query-string params baked into the iframe
   `src` at mount time.

   GetQuoteForm instead loads a local, same-origin file
   (public/forms/get-quote.html) that already wraps Zoho's
   web-to-record form. Because it's same-origin, we can talk
   to it with postMessage: GetQuoteForm sends a
   `{ type: 'bigin-prefill', payload }` message (and retries
   until the iframe acks it), and the page's own tracking
   attribution below is passed straight in via `overrides`.
   ========================================================= */
...
const EnquiryForm: React.FC<{ tracking: TrackingData; ready: boolean }> = ({ tracking, ready }) => {
  ...
  const height = isMobile ? FORM_HEIGHT_MOBILE : FORM_HEIGHT;

  /* Only the four fields GetQuoteForm's handshake actually prefills */
  const overrides = useMemo(
    ...
    [tracking.leadPageUrl, tracking.utmSource, tracking.utmCampaign, tracking.utmContent]
  );
...
export default function Home() {
  ...
  // the form waits for this so it loads exactly once, with attribution ready
